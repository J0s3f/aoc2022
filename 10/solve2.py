import re
from collections.abc import Callable


class CRT:
    def __init__(self):
        self.image = None
        self.lines = 6
        self.columns = 40
        self.cycles = self.lines * self.columns
        self.reset_image()

    def reset_image(self):
        self.image = [['.' for _ in range(self.columns)] for _ in range(self.lines)]

    def draw(self, cycle: int, x_pos: int):
        pos = (cycle % self.cycles) - 1
        if pos == 0:
            self.reset_image()
        line, column = divmod(pos, self.columns)
        if abs(x_pos - column) < 2:
            self.image[line][column] = '#'

    def __str__(self):
        return "\n".join(["".join(l) for l in self.image]) + "\n"


class CPU:
    def __init__(self):
        self.cycle = 0
        self.x = 1
        self.next_break = 20
        self.break_every = 40
        self.diagnostics = []
        self.crt = CRT()

    def run_cycle(self, op: Callable[[], None], num: int):
        for _ in range(num):
            self.cycle += 1
            if self.cycle == self.next_break:
                self.diagnostics.append((self.next_break, self.x))
                self.next_break += self.break_every
            self.crt.draw(self.cycle, self.x)
        op()

    def addx(self, val: int):
        def op():
            self.x += val

        self.run_cycle(op, 2)

    def noop(self):
        self.run_cycle(lambda: None, 1)

    def __str__(self):
        return "CPU at Cycle " + str(self.cycle) + " with registers: X: " + str(self.x)

    def run_instruction(self, instruction: str):
        parts = re.match(r"\W*(\w+)(?:[^a-zA-Z0-9_-]+(-?\d+))?", instruction)
        instruction = parts.group(1)
        operand = parts.group(2)
        match instruction:
            case 'noop':
                self.noop()
            case 'addx':
                self.addx(int(operand))

    def get_signal_strength_total(self):
        return sum([x[0] * x[1] for x in self.diagnostics])


with open('input', 'r') as file:
    lines = file.readlines()

cpu = CPU()
for line in lines:
    cpu.run_instruction(line)

print(cpu.crt)
